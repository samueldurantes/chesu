...
const GameDisplay = ({ san, bottomPlayer, topPlayer }: GameDisplayProps) => {

  const moves: { white: string, black?: string }[] = san.map((_, i) => ({ white: san[i], black: san[i + 1] })).filter((_, i) => i % 2 == 0);
    ...
    <div className="flex flex-col self-center m-4">
      <div className="text-white w-72 rounded bg-[#303030]">
        ...
        <div className="overflow-auto h-96">
          {moves.map(({ white, black }, index) => <MoveBox key={index} index={index} white={white} black={black} />)}
        </div>
